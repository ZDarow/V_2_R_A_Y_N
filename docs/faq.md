...
> **См. также:**
> - [Установка](INSTALL.md) — руководство по установке
> - [Использование](USAGE.md) — ежедневная работа
> - [Устранение неполадок](TROUBLESHOOTING.md) — решение проблем
> - [Конфигурация](CONFIGURATION.md) — файлы роутинга
...
**Подробнее:** [TROUBLESHOOTING.md](TROUBLESHOOTING.md#v2rayn-не-запускается-после-установки).
...
|-------|----------------|-------------|
| Loyalsoldier | MaxMind GeoLite2 | Стандартный, идёт в составе v2rayN/v2rayNG |
...
- Автоматически: через `update-rules.sh` (Linux)
...
Да. install.sh импортирует публичные подписки, но вы можете добавить
свои через GUI v2rayN. Ваши подписки не будут перезаписаны
при повторном запуске install.sh (используется `INSERT OR IGNORE`).

### Почему BitTorrent в only_blocked.json идёт напрямую?

BitTorrent трафик через VPN:
