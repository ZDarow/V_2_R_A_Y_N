...
# ---- Dry-run helper ----
# ВАЖНО: пишем в >&3, а не >&2, потому что log_init() (lib/common.sh:38)
# перенаправляет stderr (fd 2) в лог-файл. Без этого dry-run сообщения
# были бы не видны пользователю.
DRY_RUN=false
run() {
...
if command -v dotnet &>/dev/null; then
  DOTNET_VER=$(dotnet --version 2>/dev/null | cut -d. -f1) || true
  # shellcheck disable=SC2015
  info ".NET Runtime найден: $(dotnet --version 2>/dev/null || echo 'неизвестно')" 2>/dev/null || true
        ...
        WHITELIST-IPS-001)
          sqlite3 "$DB_PATH" "INSERT OR IGNORE INTO SubItem (Id, Remarks, Url, MoreUrl, Enabled, Sort, AutoUpdateInterval, ConvertTarget)
            VALUES ('WHITELIST-IPS-001', 'Whitelist IP (hxehex) — CIDR для мобильных',
              'https://raw.githubusercontent.com/hxehex/whitelist-ips/refs/heads/main/whitelist-ip.txt',
              '',
              1, 4, 1440, 'v2ray');" 2>/dev/null || warn "  WHITELIST-IPS-001: ошибка импорта" ;;
      esac
    done
...
if command -v v2rayn &>/dev/null; then
  if [ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]; then
    ...
    echo "  🔄 Для выключения: ~/.local/share/v2rayN/scripts/proxy-toggle.sh off"
  else
...
echo -e "  ${YELLOW}Выключить прокси:${NC}    ~/.local/share/v2rayN/scripts/proxy-toggle.sh off"
