...
echo "  Дистр:  $(lsb_release -ds 2>/dev/null || cat /etc/os-release 2>/dev/null | head -1 || echo 'N/A')"
echo "  Ядро:   $(uname -r)"
echo "  Архит:  $(uname -m)"
...
if [ -f "$DB_PATH" ] && command -v sqlite3 &>/dev/null; then
  echo "  Размер: $(ls -lh "$DB_PATH" | awk '{print $5}')"
  echo "  Таблицы:"
...
for f in routing-russia.json only_blocked.json v2rayng-routing-russia.json "v2rayng-only-blocked.json" config-template-xray.json; do
  ...
  if [ -f "$FILE" ]; then
    ...
    echo "  $f: $(ls -lh "$FILE" | awk '{print $5}') — JSON: $VALID"
  else
    warn "  $f: отсутствует"
  fi
done
...
for f in geoip.dat geosite.dat; do
  ...
  if [ -f "$FILE" ]; then
    ...
    echo "  $f: $(ls -lh "$FILE" | awk '{print $5}'), $AGE дней"
  else
    warn "  $f: отсутствует"
  fi
done
