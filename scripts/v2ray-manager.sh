...
check_status() {
    ...
    # 1. Systemd
    sec "1. Systemd юниты"
    local state; state=$(systemctl --user is-active "$SERVICE" 2>/dev/null)
    ...
    if [[ -f "$CONFIG" ]]; then
        if jq empty "$CONFIG" 2>/dev/null; then
            ok "JSON валиден"
            local inbounds; inbounds=$(jq '.inbounds | length' "$CONFIG" 2>/dev/null)
            local outbounds; outbounds=$(jq '.outbounds | length' "$CONFIG" 2>/dev/null)
            ok "Inbounds: $inbounds | Outbounds: $outbounds"
            local active; active=$(jq -r '.outbounds[] | select(.tag=="proxy") | .settings.vnext[0].address // empty' "$CONFIG" 2>/dev/null)
            if [[ -n "$active" ]]; then ok "Активный сервер: $active"; else warn "Не найден outbound .proxy."; fi
        else
            fail "JSON невалиден!"
        fi
    else
    ...
    if ss -tln 2>/dev/null | grep -q ":10808 "; then
        local code; code=$(curl -sS -o /dev/null -w "%{http_code}" --socks5-hostname 127.0.0.1:10808 -m 5 https://www.google.com 2>/dev/null)
    ...
    # 7. IP
    sec "7. Публичные IP"
    local direct_ip; direct_ip=$(curl -sS -m 5 https://ipinfo.io/ip 2>/dev/null)
    if [[ -n "$direct_ip" ]]; then ok "Напрямую: $direct_ip"; else warn "Не удалось получить прямой IP"; fi
    
    if ss -tln 2>/dev/null | grep -q ":10808 "; then
        local proxy_ip; proxy_ip=$(curl -sS -m 5 --socks5-hostname 127.0.0.1:10808 https://ipinfo.io/ip 2>/dev/null)
        if [[ -n "$proxy_ip" ]]; then
    ...
    if [[ "$mode" == "none" ]]; then
        ...
        local host; host=$(gsettings get org.gnome.system.proxy.socks host 2>/dev/null | tr -d "'")
        local port; port=$(gsettings get org.gnome.system.proxy.socks port 2>/dev/null)
...
gui_mode() {
    ...
    while true; do
        local status; status=$(systemctl --user is-active "$SERVICE" 2>/dev/null)
        local proxy; proxy=$(gsettings get org.gnome.system.proxy mode 2>/dev/null | tr -d "'")
        ...
        if [[ "$proxy" == "manual" ]]; then prx_text="✅ Прокси: ВКЛ"; else prx_text="❌ Прокси: ВЫКЛ"; fi
        
        local ip_text; ip_text=$(curl -s -m 3 --socks5-hostname 127.0.0.1:10808 https://ipinfo.io/ip 2>/dev/null || echo 'нет')
        
        local choice; choice=$(zenity --width=450 --height=300 --list --title="Менеджер прокси v2rayN" \
